...
// No native file dialog on Android; the game data lives at a fixed storage
// path, so PickIsoFile() is a no-op (see below).
#else
#include <gtk/gtk.h>
#endif
...
#elif defined(__ANDROID__)
std::filesystem::path PickIsoFile() {
  // Android has no file picker here; the game dump is provided at a fixed
  // external-storage path and installed out of band.
  return {};
}
#else
std::filesystem::path PickIsoFile() {
